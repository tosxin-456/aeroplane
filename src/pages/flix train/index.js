...
export default function TrainSearchPage() {
    ...
    const prices = filteredResults.map((trip) => trip.price.total_with_platform_fee);
    const durations = filteredResults.map(
        (trip) => trip.duration.hours + trip.duration.minutes / 60
    );
    ...
    const searchTrips = async () => {
        ...
        const url = `https://global.api.flixbus.com/search/service/v4/search?from_city_id=${fromCityId}&to_city_id=${toCityId}&departure_date=${formattedDate}&products=%7B"adult"%3A1%7D&currency=EUR&locale=en&search_by=cities&include_after_midnight_rides=1&disable_distribusion_trips=0&disable_global_trips=0&origin_flix_train=1`;
        ...
        try {
            ...
            if (data.trips && data.trips.length > 0) {
                ...
                // 🔍 Log all trips that have at least one train leg
                const tripsWithTrainLegs = allTrips.filter(
                    trip =>
                        Array.isArray(trip.legs) &&
                        trip.legs.some(leg => leg.transport_type === "train")
                );

                console.log("Trips with any train in them:", tripsWithTrainLegs);

                // ✅ Strictly FlixTrain-only trips
                const trainTrips = allTrips.filter(
                    trip =>
                        trip.provider === "flixtrain" &&
                        Array.isArray(trip.legs) &&
                        trip.legs.every(leg => leg.transport_type === "train")
                );

                setResults(trainTrips);
            } else {
                setResults([]);
            }
        } catch (err) {
            ...
            <div className="max-w-4xl mx-auto space-y-8">
                ...
                <div className="text-center space-y-2">
                    <div className="flex items-center justify-center gap-2 text-blue-600">
                        <Train size={32} />
                        <h1 className="text-3xl font-bold">European Train Search</h1>
                    </div>
                    <p className="text-gray-600">
                        Find the best train routes for your journey
                    </p>
                </div>
